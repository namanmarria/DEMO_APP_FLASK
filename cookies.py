...
class orderlogger:
    ...
    def save_log(self):
        ...
        self.units_sold = int(input("Enter number of units sold : "))
        self.date = str(datetime.datetime.today().date())
        self.revenue = float(input("Enter Revenue Generated : "))
        self.cost = float(input("Enter the cost amount : "))

        sql = "insert into orders values('{customerid}','{orderid}','{product}',{units_sold},'{date}',{revenue},{cost})".format_map(
            vars(self))
        print(sql)
